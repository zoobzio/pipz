...
### Circuit Breaker
```go
protected := pipz.NewCircuitBreaker("service", processor, 5, 30*time.Second).
    SetSuccessThreshold(2) // Optional: require 2 successes to close from half-open
```

### Rate Limiting (Singleton!)
```go
var limiter = pipz.NewRateLimiter[T]("api", 100, 10). // 100/sec, burst 10
...
```go
pipz.NewHandle("recover", pipeline,
    pipz.Effect("handle", func(ctx context.Context, pipeErr *pipz.Error[T]) error {
...
func (m *Mock[T]) Name() pipz.Name { return "mock" }
...
var pipeErr *pipz.Error[T]
if errors.As(err, &pipeErr) {
    // Path contains the full chain, last element is where failure occurred
    assert.Equal(t, pipz.Name("expected-stage"), pipeErr.Path[len(pipeErr.Path)-1])
}
```
